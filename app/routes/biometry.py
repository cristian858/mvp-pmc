...
def capture_id():
    ...
    try:
        ...
        if not ocr_result['success']:
            return jsonify({
                'error': 'No se pudo validar el nombre del documento.',
        ...
        if not name_match:
            return jsonify({
                'error': 'El nombre del documento no coincide con el usuario registrado.',
                'registered_name': current_user.name,
                'matched_name_parts': detected_name,
                'document_text_preview': ocr_result['text'][:500],
            }), 422
        ...
        return jsonify({
            ...
            'message': 'Documento capturado. Procede con la selfie y movimiento.',
        ...
        db.session.rollback()
        return jsonify({'error': f'Error capturando documento: {str(e)}'}), 500
